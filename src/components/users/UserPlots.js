...
const UserPlots = ({ user }) => {
  ...
  useEffect(() => {
    const fetchSubscriptions = async () => {
      try {
        const response = await fetch(`https://https://musabaha-homes.onrender.com/api/subscriptions?email=${user.email}`);
...
const handleProceedToPayment = (subscription) => {
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('hhttps://https://musabaha-homes.onrender.com/api/user-payments', {
        method: 'POST',
        body: formData
      });
      ...
      if (result.success) {
        ...
        const subscriptionsResponse = await fetch(`https://https://musabaha-homes.onrender.com/api/subscriptions?email=${user.email}`);
