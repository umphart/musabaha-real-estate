...
const AdminUserPayments = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await fetch("https://musabaha-homes-ltd.com.ng/api/user-payments");
  ...
  const handlePaymentAction = async (paymentId, action) => {
    try {
      ...
      const response = await fetch(
        `https://musabaha-homes-ltd.com.ng/api/user-payments/${paymentId}/status`,
        {
          method: "PATCH",
              ...
              {payments.map((p) => (
                <tr key={p.id} style={trStyles}>
                  ...
                  <td style={tdStyles}>
                    {p.receipt_file ? (
                      <a
                        href={`https://musabaha-homes-ltd.com.ng/uploads/receipts/${p.receipt_file}`}
                        target="_blank"
